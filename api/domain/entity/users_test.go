...
func testUserToManyMenus(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadMenus(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpMenus(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `FirstName`: `varchar`, `LastName`: `varchar`, `Age`: `bigint`, `PhoneNumber`: `varchar`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`, `DeletedAt`: `datetime`}
	_           = bytes.MinRead
)
